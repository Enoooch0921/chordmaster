...
- 歌單新增連續歌詞排版，多首歌曲依序流入兩欄與後續頁面，歌名與首句保持在一起，支援原位編輯、閱讀及整份 PDF 匯出
- 新增聚會楷體樣式（16 pt 字級、30 pt 行距、灰底歌名）；歌詞隱藏標點，保留原始換行，長行優先在空白處自動折行
- 歌詞頁支援所見即所得段落編輯，點選原位輸入，點外面完成或按 Esc 取消，沿用歌曲與歌單的復原／重做
- 新增段落標記替換、移動、複製、刪除、段前分頁及整首歌詞貼上
- 中英文以同一段落儲存，舊歌詞依標記配對，提示待確認段落並支援交換英文對照
- 新增中文／英文／中英對照顯示選擇、獨立的閱讀與 A4 字級／行距；手機預設大字單欄閱讀

### Changed

- 節奏與簡譜以整拍細框標示目前拍位，框內細分音符／插入位置以段落色箭頭定位
- 單首歌詞沿用聚會楷體；連續歌單標題與歌名加粗
- 和弦編輯的選取框、小節多選與輸入位置提示跟隨所屬段落配色
- 單曲與連續歌單均以無標點歌詞呈現，保留原始換行，僅在行寬不足時優先於空格處折行；段落標籤更靠近下方歌詞，與上一段拉開距離
- 歌詞 Verse 維持靠左，其餘有標籤段落的整段歌詞縮排一個中文字寬，標籤位置不變，換行與分頁同步計算
- 歌詞段落標題與和弦譜共用段落配色，閱讀、編輯與匯出保持一致
- 歌詞段落使用獨立英文標題（Verse、Pre-Chorus、Chorus、Bridge 等），相容舊數字／符號及方括號標題
- 參考實際歌詞 PDF，A4 預設歌詞字級調整為 14 pt、行距 22 pt，分頁保留標題與首行

### Fixed

- 修正選取提示遮住十六分音符：音符繪於提示上層，箭頭置於下方，整拍細框不遮擋符頭與雙橫線
- 歌詞按瀏覽器實際折行自動分欄分頁，長段落跨頁保留全文，取消單頁裁切
- 歌詞模式的歌曲、整份歌單 PDF 與全屏演出使用歌詞分頁，匯出排除編輯控件

## 0.9.8 - 2026-09-07

### Added

- 新增依帳號與團隊隔離的本機草稿、恢復視窗、衝突確認與備份下載
- 新增畫面錯誤復原介面，可重試或下載目前資料備份
- 新增團隊邀請收件匣、通知及接受／婉拒操作，改善手機團隊管理介面
- 發布及 pull request 流程加入型別檢查、自動測試與建置驗證

### Fixed

- 手機與平板匯出 PDF 只分享檔案，不再附加分享標題、文字或網址
- 離線同步改為記錄明確刪除的項目與最後已知版本，避免誤刪其他裝置新增或更新的資料
- 待同步資料依帳號與曲庫隔離，舊版備份需確認帳號後恢復
- 強化本機儲存驗證、備份回復、離開頁面保存及儲存失敗後重試
- 修正多小節休止按鈕的事件參數，以及編輯器與譜面相關型別問題
- 符號測試頁保留在本機，排除於雲端同步與分享內容之外

### Changed

- 分享頁、編輯器與 PDF 工具改為按需載入，建置時產生完整離線快取清單
- 拆出 PDF、同步合併及團隊草稿模組，補齊 React 型別並整理依賴套件
- 納入團隊邀請、未被引用的雲端測試頁清理及資料庫函式執行權限 migration

## 0.9.7 - 2026-08-21

### Added

- 新增預覽譜面多小節選取：可用 `Command/Ctrl + click` 選取多個小節，右鍵開啟批量操作選單
- 新增選取小節批量複製／貼上；貼到空白小節時會直接填入，非空白小節可選擇貼到前方或後方
- 新增多小節房子快捷鍵，選取多個小節後可用 `Option + 1..9` 批量切換房子記號，沒有多選時保留原本單小節快捷鍵行為

### Changed

- About / 更新紀錄整合 `0.9.6` 已發布但未顯示在最新版本卡片中的團隊曲庫、小節高度與節奏幾何更新

## 0.9.6 - 2026-08-19

### Added

- 新增小節高度 2 行／3 行切換；3 行模式會在畫面與 PDF 保留較高的手寫標記空間，並支援共享歌單的顯示偏好
- 團隊曲庫重整為四級角色：團隊擁有者、歌曲管理員、歌單協作者與僅可檢視，並由 RLS / RPC 驗證所有寫入權限
- 新增歌單協作者指派，只有團隊擁有者或合格的歌單建立者可指派同隊的歌曲管理員／歌單協作者
- 新增可搜尋、多選與全選的「從個人區匯入」流程，以來源歌曲 ID 追蹤覆蓋目標，並支援同名副本與舊資料連結
- 新增團隊歌曲封存／還原與受引用保護的永久刪除；封存歌曲不再出現於新增搜尋，既有歌單仍可顯示
- 新增團隊曲庫 Realtime 訂閱與視窗聚焦補充檢查；編輯中會先提示更新，不會覆蓋未儲存內容

### Changed

- 節奏預覽改用 Bach 音樂字型配合實際字形邊界建立統一幾何錨點，橫槓、符桿、符頭與三連音不再依賴逐一偏移值
- 簡譜三連音標記改為左右分段橫線，並讓非三連音的小節維持緊湊高度
- 個人歌曲匯入團隊後永遠保留原歌名，不再附加「個人匯入」尾碼
- 團隊一般刪除改為封存，批次匯入改為交易式 RPC，並收緊專案、歌單、成員與跨曲庫關聯權限

### Fixed

- 修正節奏重音、單一八分音符、連音橫槓與三連音的符頭／符桿對齊問題
- 修正同小節及跨小節連接線的端點高度與弧度，使兩端落在符頭下方並保持一致
- 修正反覆房子標籤過高及橫線偏離原小節位置的問題，畫面與 PDF 使用一致基準
- 分享連結必須先通過目前權限檢查，不再於授權前回傳既有 token

## 0.9.5 - 2026-07-27

### Fixed

- 修正分享歌單與共享專案載入時，若原歌庫歌曲仍存在會優先顯示原歌曲內容，而不是歌單內 `songData` 副本的問題；現在歌單中新增／刪除段落後，分享對象也會看到同一份段落結構

## 0.9.4 - 2026-07-27

### Fixed

- 修正點擊已有節奏內容的灰色節奏 lane 時，節奏元件攔截事件但沒有把 click event 傳回上層，導致節奏鍵盤沒有打開的問題

## 0.9.3 - 2026-07-27

### Fixed

- 修正完成預覽輸入後重新點回節奏時，節奏鍵盤可能因沿用 lower lane 目標而無法重新打開的問題
