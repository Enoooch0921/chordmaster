...
- 新增預覽鍵盤的小節和弦、簡譜與節奏複製／貼上操作，並用不同顏色區分不同類型的複製貼上按鈕
- 新增和弦硬體快捷鍵：`Space` 跳到下一拍、`Enter` 跳到下一小節、`Shift+Space` 在目前拍前插入一拍、`Shift+Enter` 在目前小節前插入小節
- 新增簡譜輸入時值與附點的語意容量判斷，允許 `八分 + 四分 + 八分` 正確跨滿兩拍

### Changed

- 段落標題改為只在使用者主動換行時分行，並自動把每個單詞首字母大寫
- 簡譜按附點時，如果游標在前一個音符後方，會優先嘗試替前一個音符加附點並繼續往後走
- 預覽放大時移除容易造成文字模糊的 raster 化提示，並調整文字與 SVG 渲染清晰度

### Fixed

- 修正簡譜第一拍、空游標與跨拍插入時誤判「沒有足夠空位」的問題
- 修正簡譜連接線可能連到前面錯誤音符的問題，現在會依選取音符的語意位置決定連線端點
- 修正和弦貼上可能貼到非目前選取小節的問題，複製／貼上操作會以目前預覽編輯目標為準
...
- 歌單曲目保留「歌單 Key／Capo」標示，頂端維持精簡 Key／Capo 控制，兩處共用原有更新與權限流程
