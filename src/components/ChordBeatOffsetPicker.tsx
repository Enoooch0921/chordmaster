...
export default function ChordBeatOffsetPicker({ value, disabled, onChange, language }: {
  value: ChordMark['beatOffset']; disabled?: boolean; onChange: (value: number) => void; language: AppLanguage;
}) {
  return <label className="flex min-w-0 items-center gap-1 text-[11px] font-semibold text-slate-700">
    ...
    <select aria-label={language === 'zh' ? '和弦拍內位置' : 'Chord beat position'} value={value ?? 0} disabled={disabled}
      ...
      <option value="0.25">e · +¼</option><option value="0.5">&amp; · +½</option><option value="0.75">a · +¾</option>
    </select>
  </label>;
}
