-- Final team-library hardening. Deploy only after the additive 090000
-- compatibility migration and its matching frontend have been deployed.
--
-- This is intentionally forward-only. Do not rewrite earlier migrations: the
-- production migration ledger must be reconciled before this file is pushed.
...
-- The additive migration normally created this key. Keep this fallback
-- idempotent without dropping a key already referenced by the assignment FK.
do $$
begin
  if not exists (
    select 1
    ...
    where conrelid = 'public.setlists'::regclass
      and conname = 'setlists_id_library_id_key'
  ) then
...
-- Complete before/after values are retained so an operator can construct a
-- deterministic rollback without rewriting migration history.
create table if not exists public.team_song_legacy_title_cleanup_log (
  ...
  setlist_overrides jsonb not null default '[]'::jsonb,
  cleaned_at timestamptz not null default now(),
  unique (migration_key, song_id)
);
...
alter table public.team_song_legacy_title_cleanup_log enable row level security;
...
revoke all on table public.team_song_legacy_title_cleanup_log from anon, authenticated;

-- ---------------------------------------------------------------------------
-- 2. Owner, assignment, and cross-library invariants
-- ---------------------------------------------------------------------------
...
-- Repair any historic owner membership before enforcing the invariant on all
-- future member writes. Extra historic owner rows are safely demoted.
insert into public.library_members (library_id, user_id, role)
select l.id, l.owner_user_id, 'owner'
from public.libraries l
on conflict (library_id, user_id) do update set role = 'owner';

update public.library_members lm
set role = case when l.kind = 'team' then 'editor' else 'viewer' end
from public.libraries l
where l.id = lm.library_id
  and lm.role = 'owner'
  and lm.user_id <> l.owner_user_id;

create or replace function public.enforce_library_member_owner_invariant()
returns trigger
language plpgsql
security definer
...
begin
  ...
  if new.role = 'owner' and new.user_id <> v_owner_user_id then
    raise exception 'Only libraries.owner_user_id may hold the owner role';
  end if;

  if new.user_id = v_owner_user_id and new.role <> 'owner' then
    raise exception 'The library owner membership must keep the owner role';
  end if;
...
-- ---------------------------------------------------------------------------
-- 5. Setlist-assignment and membership RPCs
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_kind <> 'team' or not public.can_manage_library_members(p_library_id) then
    raise exception 'Access denied';
  end if;
...
set search_path = public
as $$
  select public.can_manage_library_members(target_library_id)
$$;
...
create or replace function public.can_manage_share_resource(
...
set search_path = public
as $$
  select case target_resource_type
    when 'song' then exists (
      select 1
      from public.songs s
      where s.id = target_resource_id
        and public.can_edit_library_content(s.library_id)
    )
    when 'song_bundle' then exists (
      select 1
      from public.song_share_bundles b
      where b.id = target_resource_id
        and b.created_by = auth.uid()
        and public.can_edit_library_content(b.library_id)
    )
    when 'setlist' then exists (
      select 1
      from public.setlists sl
      where sl.id = target_resource_id
        and public.can_edit_library_content(sl.library_id)
        and public.can_write_setlist(sl.id)
    )
    when 'project' then exists (
      ...
      where p.id = target_resource_id
        and (
          (
            public.can_edit_library_content(p.library_id)
            and public.can_write_project(p.id)
          )
          or public.has_project_manager_role(p.id)
        )
    )
    else false
  end
...
-- Members can read libraries, but direct member-table mutation is limited to
-- the personal-library self-owner bootstrap used by existing clients. Team
-- membership changes must go through the checked RPCs.
drop policy if exists "library_members_owner_write" on public.library_members;
drop policy if exists "library_members_self_owner_insert" on public.library_members;
create policy "library_members_self_owner_insert" on public.library_members
...
drop policy if exists "library_members_self_owner_update" on public.library_members;
create policy "library_members_self_owner_update" on public.library_members
for update using (
  user_id = auth.uid()
  and role = 'owner'
  and exists (
...
drop policy if exists "songs_insert_by_library_manager" on public.songs;
create policy "songs_insert_by_library_manager" on public.songs
for insert with check (
  public.can_edit_library_content(library_id)
...
drop policy if exists "songs_update_by_library_manager" on public.songs;
create policy "songs_update_by_library_manager" on public.songs
for update using (public.can_edit_library_content(library_id))
with check (
  public.can_edit_library_content(library_id)
  and updated_by = auth.uid()
);

drop policy if exists "songs_delete_personal_owner" on public.songs;
create policy "songs_delete_personal_owner" on public.songs
for delete using (
  public.can_edit_library_content(library_id)
  and exists (
    select 1
...
for all using (public.can_write_setlist(setlist_id))
with check (public.can_write_setlist(setlist_id));

drop policy if exists "projects_editor_write" on public.projects;
drop policy if exists "projects_insert_by_role" on public.projects;
create policy "projects_insert_by_role" on public.projects
for insert with check (
  public.can_create_setlist(library_id)
  and created_by = auth.uid()
  and updated_by = auth.uid()
...
create policy "projects_update_by_role" on public.projects
for update using (public.can_write_project(id))
with check (
  public.can_write_project(id)
  and updated_by = auth.uid()
);

drop policy if exists "projects_delete_by_role" on public.projects;
create policy "projects_delete_by_role" on public.projects
for delete using (public.can_write_project(id));

create or replace function public.enforce_share_link_immutability()
returns trigger
language plpgsql
as $$
begin
  if new.id is distinct from old.id
    ...
    raise exception 'A share link resource, token, and creator are immutable';
  end if;
  return new;
end;
$$;

drop trigger if exists share_links_immutability on public.share_links;
create trigger share_links_immutability
before update on public.share_links
for each row execute function public.enforce_share_link_immutability();
...
for select using (
  created_by = auth.uid()
  and public.can_manage_share_resource(resource_type, resource_id)
);

drop policy if exists "share_links_owner_write" on public.share_links;
drop policy if exists "share_links_authorized_insert" on public.share_links;
create policy "share_links_authorized_insert" on public.share_links
for insert with check (
  created_by = auth.uid()
  and public.can_manage_share_resource(resource_type, resource_id)
);

drop policy if exists "share_links_authorized_update" on public.share_links;
create policy "share_links_authorized_update" on public.share_links
for update using (
  created_by = auth.uid()
  and public.can_manage_share_resource(resource_type, resource_id)
) with check (
...
drop policy if exists "share_links_authorized_delete" on public.share_links;
create policy "share_links_authorized_delete" on public.share_links
for delete using (
  created_by = auth.uid()
  and public.can_manage_share_resource(resource_type, resource_id)
);
...
drop trigger if exists library_members_owner_invariant on public.library_members;
create trigger library_members_owner_invariant
before insert or update of library_id, user_id, role on public.library_members
for each row execute function public.enforce_library_member_owner_invariant();
...
-- Defensive cleanup for any role changes made during a staged deployment
-- between the additive migration and this hardening migration.
delete from public.setlist_editor_assignments sea
using public.library_members lm
where lm.library_id = sea.library_id
  and lm.user_id = sea.user_id
  and lm.role not in ('editor', 'setlist_manager');
...
-- ---------------------------------------------------------------------------
-- 3. Legacy imported-title cleanup with rollback evidence
-- ---------------------------------------------------------------------------

do $$
declare
  v_pattern constant text := '([[:space:]]+\((個人匯入|Personal[[:space:]]+import)([[:space:]]+[0-9]+)?\)([[:space:]]+[0-9]+)?)+[[:space:]]*$';
  v_migration_key constant text := '20260801100000_team_library_hardening';
begin
  insert into public.team_song_legacy_title_cleanup_log (
    migration_key,
    song_id,
    library_id,
    old_title,
  ...
  select
    ...
    s.content_json,
    case
      when jsonb_typeof(s.content_json) = 'object'
        then jsonb_set(s.content_json, '{title}', to_jsonb(cleaned.new_title), true)
      else s.content_json
    end,
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          'setlistSongId', ss.id,
          'oldOverrideJson', ss.override_json,
          'newOverrideJson', jsonb_set(
            ss.override_json,
            '{songData,title}',
            to_jsonb(cleaned.new_title),
            false
          )
        )
        order by ss.id
      ...
      where ss.song_id = s.id
        and jsonb_typeof(ss.override_json -> 'songData') = 'object'
        and jsonb_typeof(ss.override_json #> '{songData,title}') = 'string'
    ), '[]'::jsonb)
  from public.songs s
  join public.libraries l
  ...
  cross join lateral (
    select coalesce(
      nullif(btrim(regexp_replace(s.title, v_pattern, '', 'i')), ''),
      s.title
    ) as new_title
  ) cleaned
  where s.title ~* v_pattern
  on conflict (migration_key, song_id) do nothing;

  update public.songs s
  set title = log.new_title,
      content_json = log.new_content_json
  from public.team_song_legacy_title_cleanup_log log
  where log.migration_key = v_migration_key
    and log.song_id = s.id
    and s.library_id = log.library_id;

  update public.setlist_songs ss
  set override_json = jsonb_set(
    ss.override_json,
    '{songData,title}',
    to_jsonb(log.new_title),
    false
  )
  from public.team_song_legacy_title_cleanup_log log
  where log.migration_key = v_migration_key
    and ss.song_id = log.song_id
    and jsonb_typeof(ss.override_json -> 'songData') = 'object'
    and jsonb_typeof(ss.override_json #> '{songData,title}') = 'string';
end;
$$;
...
-- ---------------------------------------------------------------------------
-- 6. Transactional personal-to-team song import RPCs
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 7. Archive and safe permanent-delete RPCs
-- ---------------------------------------------------------------------------

-- Reference additions/retargeting and song deletion take this transaction-wide
-- gate at statement start, before PostgreSQL can lock individual rows. Pure
-- child-reference removal stays outside it. Setlist/bundle parent deletion and
-- their child cascades also stay outside it: activation follows G -> parent,
-- while deletion never waits for G after locking a parent. auth.users deletion
-- is the one ancestor exception: it takes G before locking the user because its
-- cascades can delete songs while share insertion key-locks created_by.
...
-- ---------------------------------------------------------------------------
-- 8. Preserve archived-source state in joined setlist/project payloads
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 9. Apply creator-scope to every project/setlist management RPC
-- ---------------------------------------------------------------------------
    ...
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'name', coalesce(profile.display_name, profile.email, ''),
          'picture', profile.avatar_url,
          'role', upm.role,
          'joinedAt', upm.joined_at
        ) order by upm.joined_at
      ),
      '[]'::jsonb
    )
  into v_participant_count, v_participants
  from public.user_project_memberships upm
  left join public.profiles profile on profile.id = upm.user_id
...
begin
  ...
  if not public.can_manage_share_resource('setlist', p_setlist_id) then
    raise exception 'Access denied';
  end if;
  ...
  select
    ...
    coalesce(
      jsonb_agg(
        ...
        ) order by usm.joined_at
      ),
      '[]'::jsonb
    )
  into v_participant_count, v_participants
  from public.user_setlist_memberships usm
  left join public.profiles profile on profile.id = usm.user_id
...
begin
  ...
  delete from public.user_setlist_capo_overrides override_row
  using public.setlist_songs setlist_song
  where override_row.setlist_song_id = setlist_song.id
    and setlist_song.setlist_id = p_setlist_id;
...
as $$
  with setlist_contacts as (
    ...
    from public.user_setlist_memberships usm
    join public.setlists sl on sl.id = usm.setlist_id
    where public.can_manage_share_resource('setlist', sl.id)
      and usm.user_id <> auth.uid()
  ), project_contacts as (
    select distinct upm.user_id
    from public.user_project_memberships upm
    join public.projects p on p.id = upm.project_id
    where public.can_manage_share_resource('project', p.id)
  ...
  from contacts
  left join public.profiles profile on profile.id = contacts.user_id
$$;
...
begin
  ...
  if p_resource_type = 'setlist' then
    ...
    if not public.can_manage_share_resource('setlist', p_resource_id) then
      raise exception 'Access denied';
    end if;
    ...
    if not public.can_manage_share_resource('project', p_resource_id) then
      raise exception 'Access denied';
    end if;
  else
...
begin
  ...
  if p_resource_type = 'setlist' then
    delete from public.user_setlist_memberships
    where setlist_id = p_resource_id
      and user_id = p_user_id;
  else
    delete from public.user_project_memberships
    where project_id = p_resource_id
      and user_id = p_user_id;
  end if;
...
-- ---------------------------------------------------------------------------
-- 10. Function privileges and Realtime publication
-- ---------------------------------------------------------------------------

revoke all on function public.enforce_library_row_integrity() from public;
revoke all on function public.enforce_library_member_owner_invariant() from public;
...
revoke all on function public.enforce_share_link_immutability() from public;
...
revoke all on function public.can_manage_share_resource(text, text) from public;

revoke all on function public.can_read_library(text) from public;
revoke all on function public.can_edit_library_content(text) from public;
revoke all on function public.can_write_library(text) from public;
revoke all on function public.can_create_setlist(text) from public;
revoke all on function public.can_manage_library_members(text) from public;
revoke all on function public.is_library_owner(text) from public;
...
grant execute on function public.can_read_library(text) to authenticated;
grant execute on function public.can_edit_library_content(text) to authenticated;
grant execute on function public.can_write_library(text) to authenticated;
grant execute on function public.can_create_setlist(text) to authenticated;
grant execute on function public.can_manage_library_members(text) to authenticated;
grant execute on function public.is_library_owner(text) to authenticated;
...
grant execute on function public.can_manage_share_resource(text, text) to authenticated;
...
revoke all on function public.get_joined_setlists() from public;
revoke all on function public.get_joined_projects() from public;
...
-- Existing public RPC signatures retain their ACL across CREATE OR REPLACE,
-- but state the intended grants explicitly for reviewability.
...
grant execute on function public.get_setlist_share_status(text) to authenticated;
grant execute on function public.revoke_setlist_sharing(text) to authenticated;
...
-- DELETE payloads must retain routing fields that are not always part of the
-- primary key (notably assignment.library_id) for scoped Realtime refreshes.
